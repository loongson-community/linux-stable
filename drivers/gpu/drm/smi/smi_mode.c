...
#if LINUX_VERSION_CODE > KERNEL_VERSION(3,14,0)	
	int pitch = crtc->primary->fb->pitches[0] ;
#else
...
int smi_connector_get_modes(struct drm_connector *connector)
{
	int count, ret;
	void *tmpedid;
	struct smi_device *smi_device = connector->dev->dev_private;
	...
	if(g_specId == SPC_SM750)
	{
...
#else
			tmpedid = &smi_device->edid[0];
			ret = ddk750_edidReadMonitorEx_HW(SMI0_CTRL, tmpedid, 256, 0);
			dbg_msg("DVI edid size= %d\n",ret);
			if (ret) {
#if LINUX_VERSION_CODE > KERNEL_VERSION(4,18,0)
				drm_connector_update_edid_property(connector, &smi_device->edid[0]);
#else
				drm_mode_connector_update_edid_property(connector, &smi_device->edid[0]);
#endif
				count = drm_add_edid_modes(connector, &smi_device->edid[0]);
			}
			if (ret == 0 || count == 0) {
#if LINUX_VERSION_CODE > KERNEL_VERSION(4,18,0)
		...
		{
			tmpedid = &smi_device->edid[1];
			ret = ddk750_edidReadMonitorEx(SMI1_CTRL, tmpedid, 256, 0, 17, 18);

			if (ret) {
#if LINUX_VERSION_CODE > KERNEL_VERSION(4,18,0)
				drm_connector_update_edid_property(connector, &smi_device->edid[1]);
#else
				drm_mode_connector_update_edid_property(connector, &smi_device->edid[1]);
#endif
				count = drm_add_edid_modes(connector, &smi_device->edid[1]);
			}
			if (ret == 0 || count == 0) {
#if LINUX_VERSION_CODE > KERNEL_VERSION(4,18,0)
				drm_connector_update_edid_property(connector, NULL);
	...
	{ //SM768 Part
		...
		if(connector->connector_type == DRM_MODE_CONNECTOR_HDMIA)
		{
			...
			if (ret) {
#if LINUX_VERSION_CODE > KERNEL_VERSION(4,18,0)
				drm_connector_update_edid_property(connector, &smi_device->edid[2]);
#else
				drm_mode_connector_update_edid_property(connector, &smi_device->edid[2]);
#endif
				count = drm_add_edid_modes(connector, &smi_device->edid[2]);
				smi_device->is_hdmi = drm_detect_hdmi_monitor(&smi_device->edid[2]);
				dbg_msg("HDMI connector is %s\n",(smi_device->is_hdmi ? "HDMI monitor" : "DVI monitor"));
			}
			if (ret == 0 || count == 0) {
#if LINUX_VERSION_CODE > KERNEL_VERSION(4,18,0)
				drm_connector_update_edid_property(connector, NULL);
