...
void maybe_disable_cpufreq(void)
{
	if ((num_online_cpus() == 1) && (system_state != SYSTEM_BOOTING)) {
		...
		struct cpufreq_policy *pol = per_cpu(cpufreq_cpu_data, 0);
		...
		cpufreq_enabled = 0;
		cpufreq_notify_transition(pol, &freqs, CPUFREQ_PRECHANGE);
		cpuclk->rate = cpu_clock_freq / 1000;
		LOONGSON_CHIPCFG0 |= 0x7;	/* Set to highest frequency */
		cpufreq_notify_transition(pol, &freqs, CPUFREQ_POSTCHANGE);
	}
}
