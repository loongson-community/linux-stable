...
/* Initialize chip */
static void nct6102_chip_init(void)
{
	u8 tmp;

	/* Choice GP2x */
	tmp = read_gbl(SIO_REG_MFS);
	tmp &= ~(1 << 1);
	write_gbl(tmp, SIO_REG_MFS);

	/* Enable GP2x */
	tmp = read_dev(SIO_DEV_GPIO, SIO_REG_ENABLE);
	tmp |= 1 << 2;
	write_dev(tmp, SIO_DEV_GPIO, SIO_REG_ENABLE);
}
