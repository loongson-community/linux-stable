...
static int azx_first_init(struct azx *chip)
{
	...
	chip->addr = platform_resource_start(to_platform_device(chip->card->dev), 0);
	...
	if ((gcap & ICH6_GCAP_64OK) && !dma_set_mask(chip->card->dev, DMA_BIT_MASK(64)))
...
static int azx_probe(struct platform_device *pdev)
{
	...
	err = azx_create(card, pdev, dev, AZX_DRIVER_ICH | AZX_DCAPS_LS2H_WORKAROUND,
			 &loongson_hda_ops, &chip);
	if (err < 0)
		goto out_free;
