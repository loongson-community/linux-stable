...
static inline const char *plat_name(struct platform_device *pdev)
{
 ...
 again:
	...
	if (!chip->polling_mode && chip->poll_count < 2) {
		snd_printdd(SFX "%s: azx_get_response timeout, "
			   ...
			   plat_name(chip->dev), chip->last_cmd[addr]);
		do_poll = 1;
		chip->poll_count++;
		goto again;
	}
...
static int azx_single_wait_for_response(struct azx *chip, unsigned int addr)
{
	...
	if (printk_ratelimit())
		snd_printd(SFX "%s: get_response timeout: IRS=0x%x\n",
			   plat_name(chip->dev), azx_readw(chip, IRS));
...
static int azx_single_send_cmd(struct hda_bus *bus, u32 val)
{
	...
	if (printk_ratelimit())
		snd_printd(SFX "%s: send_cmd timeout: IRS=0x%x, val=0x%x\n",
			   plat_name(chip->dev), azx_readw(chip, IRS), val);
	return -EIO;
}
...
static int azx_reset(struct azx *chip, int full_reset)
{
	...
	if (!chip->codec_mask) {
		...
		snd_printdd(SFX "%s: codec_mask = 0x%x\n", plat_name(chip->dev), chip->codec_mask);
	}
...
static irqreturn_t azx_interrupt(int irq, void *dev_id)
{
	...
	if (chip->dev->dev.power.runtime_status != RPM_ACTIVE)
		return IRQ_NONE;
#endif
			     ...
			     struct azx_dev *azx_dev)
{
	...
	if (!azx_dev->no_period_wakeup && pos_adj > 0) {
		...
		if (pos_adj >= period_bytes) {
			snd_printk(KERN_WARNING SFX "%s: Too big adjustment %d\n",
				   plat_name(chip->dev), bdl_pos_adj[chip->dev_index]);
			pos_adj = 0;
		} else {
 ...
 error:
	snd_printk(KERN_ERR SFX "%s: Too many BDL entries: buffer=%d, period=%d\n",
		   plat_name(chip->dev), azx_dev->bufsize, period_bytes);
	return -EINVAL;
}
...
static int azx_pcm_prepare(struct snd_pcm_substream *substream)
{
	...
	if (!format_val) {
		snd_printk(KERN_ERR SFX
			   ...
			   plat_name(chip->dev), runtime->rate, runtime->channels, runtime->format);
		err = -EINVAL;
		goto unlock;
	}
	...
	snd_printdd(SFX "%s: azx_pcm_prepare: bufsize=0x%x, format=0x%x\n",
		    plat_name(chip->dev), bufsize, format_val);
	...
	err = ls2h_hda_codec_prepare(apcm->codec, hinfo, stream_tag,
...
static void azx_power_notify(struct hda_bus *bus, bool power_up)
{
	...
	if (power_up)
		pm_runtime_get_sync(&chip->dev->dev);
	else
		pm_runtime_put_sync(&chip->dev->dev);
}
...
{
#ifdef CONFIG_PM
	/* The codecs were powered up in ls2h_hda_codec_new().
