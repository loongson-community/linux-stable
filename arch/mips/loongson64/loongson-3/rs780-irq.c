...
unsigned int rs780_ipi_irq2pos[MAX_IRQS] = { [0 ... MAX_IRQS-1] = -1 };
unsigned int rs780_ipi_pos2irq[MAX_DIRQS] = { [0 ... MAX_DIRQS-1] = -1 };
...
void rs780_irq_dispatch(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(ht_irq); i++) {
		...
		if (rs780_ipi_irq2pos[ht_irq[i]] == -1) {
			do_IRQ(ht_irq[i]);
			continue;
		}
		...
		loongson3_send_irq_by_ipi(irq_cpu[ht_irq[i]], (0x1 << (rs780_ipi_irq2pos[ht_irq[i]])));
	}
}
...
void rs780_msi_irq_dispatch(void)
{
	...
	for (i = 0; i < 4; i++) {
		...
		while (irqs) {
			...
			/* balanced by other cores */
			loongson3_send_irq_by_ipi(irq_cpu[irq], (0x1 << (rs780_ipi_irq2pos[irq])));
		}
	}
}
...
static void create_ipi_dirq(unsigned int irq)
{
	...
	rs780_ipi_pos2irq[pos] = irq;
	rs780_ipi_irq2pos[irq] = pos;
...
static void destroy_ipi_dirq(unsigned int irq)
{
	...
	pos = rs780_ipi_irq2pos[irq];

	if (pos < 0)
		return;

	rs780_ipi_irq2pos[irq] = -1;
	rs780_ipi_pos2irq[pos] = -1;
...
void rs780_init_irq(void)
{
	...
	if (pci_msi_enabled())
		...
		for (i = 0; i < MAX_IRQS; i++)
			rs780_ipi_irq2pos[i] = -1;
		for (i = 0; i < MAX_DIRQS; i++)
			rs780_ipi_pos2irq[i] = -1;
		create_ipi_dirq(3);
		create_ipi_dirq(4);
		create_ipi_dirq(5);
		create_ipi_dirq(6);
